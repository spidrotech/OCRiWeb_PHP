<?php
	...
	class IndWaitingByQueueController extends OcriControllerChart{
		...
		public function getResult($parameters){
			...
			if(count($parameters['results'])>0){
				...
				$display=$this->getView('IndWaitingByQueueTable', $parameters);
		...
		private function setData(&$parameters){
			$flatResults=$this->adaptator->waitingByQueue($parameters['cluster'],$parameters['monthStart'],$parameters['monthEnd'],$parameters['year'],$parameters['mode']);
			$top=array();
			...
			$this->rootName.= 'IndWaitingByQueue'.'_'.$parameters['cluster']."_".$parameters['year'].'-'.$parameters['monthStart'].'_'.$parameters['yearEnd'].'-'.$parameters['monthEnd'];
		}
		...
		private function createChart($parameters){
			...
			switch($parameters['mode']){
				default:
				case 'avg':
					$titleTime='Nombre moyen d\'heures d\'attente moyen sur ';
				break;
				case 'sum': 
					$titleTime='Somme des heures d\'attente moyen sur ';
				break;
			}
			$stringParameters=$parameters['cluster']." du ".$parameters['monthStart']."-".$parameters['year'].' au '.$parameters['monthEnd']."-".$parameters['yearEnd'];
			$titleTime.=$stringParameters;
			$titleCount='Nombre mensuel de jobs en attente sur '.$stringParameters;
		...
		private function drawChart($title,$chartPath,$data){
			...
			$myPicture->drawLegend(5,585,array("Style"=>LEGEND_ROUND,"Mode"=>LEGEND_HORIZONTAL));
